...
void BasicExcel::UpdateWorksheets()
{
	...
	for(int s=0; s<maxWorksheets; ++s) {
		...
		vector<SmartPtr<Worksheet::CellTable::RowBlock::CellBlock> >* pCellBlocks = NULL;
...
wstring BasicExcelCell::GetMyString() const
{
	if (type_ == STRING) {
		wstring s(str_.begin(), str_.end());
		return s;
	}
	else {
		assert(type_ == WSTRING);
		wstring s(wstr_.begin(), wstr_.end());
		return s;
	}
}
